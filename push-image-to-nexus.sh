...
jq --arg froalaeditor "file:${PACKAGE_NAME}-${PACKAGE_VERSION}.tgz" '.dependencies["froala-editor"] |= $froalaeditor' bower.json  > new.file && cat new.file > bower.json && rm -f new.file
echo "verify package"
cat bower.json
